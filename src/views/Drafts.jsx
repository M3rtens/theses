...
import { deleteDraft as deleteLocalDraft, relativeTime } from '../lib/drafts.js'
...
const statusLabel = (draft) => {
  if (!draft.scheduledPublicationId) return draft.offline ? 'Offline draft' : 'Cloud draft'
...
export default function Drafts({ navigate }) {
  const user = useUser()
  const { drafts: cloudDrafts, scheduled, loadDrafts, loadScheduled } = useData()
  ...
  const drafts = useMemo(() => [...scheduled, ...cloudDrafts]
    .sort((a, b) => Number(b.savedAt || 0) - Number(a.savedAt || 0))
    .map((draft) => ({ ...draft, lastEdited: relativeTime(draft.savedAt) })), [cloudDrafts, scheduled])

  const mutateSchedule = async (draft, action) => {
    if (busyId) return
    setBusyId(`schedule-${draft.scheduledPublicationId}`)
    setOperationError('')
    try {
  ...
  const confirmDelete = async () => {
    ...
    const operationId = draft.scheduledPublicationId
      ? `schedule-${draft.scheduledPublicationId}`
      : `draft-${draft.cloudDraftId || draft.localDraftId || draft.id}`
    ...
    try {
      ...
      if (draft.cloudDraftId) {
        const response = await fetch(`/api/drafts/${draft.cloudDraftId}`, { method: 'DELETE' })
        if (!response.ok && response.status !== 404) {
          const data = await response.json().catch(() => null)
          throw new Error(data?.error || `HTTP ${response.status}`)
        }
      }
      deleteLocalDraft(draft.localDraftId || draft.id, user?.id)
      await loadDrafts()
    } catch (error) {
      setOperationError(error.message)
    } finally {
  ...
  return (
    <>
      <header className="px-4 pt-6 pb-5 sm:px-6 sm:pt-8 sm:pb-6 lg:px-12 border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>Private cloud drafts, available across your signed-in devices.</p>
          </div>
        ...
        <div className="mt-6 p-4 border rounded-md flex items-center gap-3" style={{ borderColor: 'var(--border)', background: 'var(--bg-warm)' }}>
          ...
          <p className="text-xs" style={{ color: 'var(--ink-soft)' }}>
            Drafts autosave to the cloud and keep a browser copy for offline safety. Conflicting edits are preserved as a separate copy. Scheduled theses remain editable until processing starts.
          </p>
        </div>
      ...
      <div className="px-4 py-6 sm:px-6 sm:py-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {drafts.map((draft) => {
            ...
            const isScheduled = Boolean(draft.scheduledPublicationId)
            const processing = draft.scheduleStatus === 'processing'
            const operationId = isScheduled
              ? `schedule-${draft.scheduledPublicationId}`
              : `draft-${draft.cloudDraftId || draft.localDraftId || draft.id}`
            const busy = busyId === operationId
            return (
              <div
                key={`${isScheduled ? 'scheduled' : 'draft'}-${draft.id}`}
                className={`thesis-card rounded-md p-5 ${processing ? '' : 'cursor-pointer'}`}
                onClick={() => openDraft(draft)}
                style={{ opacity: processing || busy ? 0.72 : 1 }}
              >
                ...
                <h3 className="font-serif text-lg font-medium mb-3 leading-snug">{draft.title || 'Untitled thesis'}</h3>
                ...
                <div className="pt-4 border-t flex flex-wrap items-center justify-between gap-3" style={{ borderColor: 'var(--border)' }}>
                  <span className="text-[10px] font-mono uppercase tracking-wider" style={{ color: draft.scheduleStatus === 'action_required' || draft.offline ? 'var(--bear)' : 'var(--muted)' }}>{status}</span>
                  <div className="flex items-center gap-3">
                    {isScheduled && ['pending', 'action_required'].includes(draft.scheduleStatus) && (
                    ...
                    {!processing && (
                      <button type="button" disabled={busy} onClick={(event) => requestDelete(event, draft)} title="Delete draft" className="text-xs font-medium flex items-center gap-1" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--bear)' }}>
      ...
      <ConfirmModal
        ...
        message={pendingDelete ? `“${pendingDelete.title || 'Untitled thesis'}” will be permanently removed. Unpublished drafts are not part of the public record.` : ''}
